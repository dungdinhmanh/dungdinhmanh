<template>
  ...
  const t = (key: string) => {
    ...
    let value = translations[selectedLanguage.value]
    for (const k of keys) {
      value = value[k]
    }
    return value || key
  ...
  const toggles = computed(() => ({
    fireworksEnabled: t('fireworksEnabled'),
    SpinePlayerEnabled: t('SpinePlayerEnabled'),
  }))
  ...
  onMounted(() => {
      ...
      // 读取其他开关状态
      const toggleKeys = ['fireworksEnabled', 'SpinePlayerEnabled']
  ...
  const changeLanguage = () => {
      localStorage.setItem('uiLanguage', selectedLanguage.value);
  };
